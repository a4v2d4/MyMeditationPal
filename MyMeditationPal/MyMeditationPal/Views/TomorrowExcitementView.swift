...
struct TomorrowExcitementView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: Theme.largePadding) {
                        ...
                        // Add more button
                        Button(action: addExcitementItem) {
                            HStack(spacing: 8) {
                        ...
                        Button(action: saveExcitement) {
                            ...
                            .padding(.vertical, 16)
                            .background(
                                completedItemsCount >= 1 ? Theme.successGreen : Color(red: 0.4, green: 0.75, blue: 0.95)
                            )
                            .cornerRadius(Theme.cardCornerRadius)
                        ...
                        if completedItemsCount > 0 {
                            HStack(spacing: 6) {
                                Image(systemName: completedItemsCount >= 1 ? "checkmark.circle.fill" : "circle.inset.filled")
                                    .font(.system(size: 16))
                                    .foregroundColor(completedItemsCount >= 1 ? Theme.successGreen : Color(red: 0.4, green: 0.75, blue: 0.95))
                                
                                Text("\(completedItemsCount) item\(completedItemsCount == 1 ? "" : "s")")
                                    .font(.system(size: 14))
                                    .foregroundColor(Theme.textSecondary)
                            }
                        }
    ...
    private func addExcitementItem() {
        excitementItems.append("")
    }
    
    private func loadExistingExcitement() {
        if let items = viewModel.loadTodayExcitementItems(), !items.isEmpty {
            excitementItems = items
        }
    }
    
    private func saveExcitement() {
        isSaving = true
        
        // Filter out empty items
        let nonEmptyItems = excitementItems.filter { !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        
        // Save excitement items
        viewModel.saveExcitementItems(nonEmptyItems)
...
struct ExcitementTextField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField("I'm excited about...", text: $text, axis: .vertical)
                .font(.system(size: 16))
                .foregroundColor(Theme.textPrimary)
...
#Preview {
    TomorrowExcitementView(viewModel: MeditationViewModel())
}
